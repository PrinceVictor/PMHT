...
def get_process_noise_matrix(delta_t, sigma=0.01):
    Q = np.power(sigma, 2) * np.array([[np.power(delta_t, 4)/4, np.power(delta_t, 3)/2, 0., 0.],
...
def get_measurement_noise_matrix(sigma=100):
    R = np.power(sigma, 2) * np.array(np.eye(2, dtype=np.float))
...
def state_predict(x, P, Q, delta_t):
    ...
    x_predict = cv_state_trans_mat @ x
    P_predict = cv_state_trans_mat @ P @ cv_state_trans_mat.T + Q
...
def state_update(x_predict, P_predict, z, R):
    ...
    k_gain = P_predict @ H.T @ inv(np.round(H @ P_predict @ H.T + R))
